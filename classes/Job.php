...
class Job
{
    ...
    public function getWork( $work_LIMIT = -1 ){ return $this->APICall( array('get' => $work_LIMIT), "Couldn't get work for " . User::$cUser->description(). " from " . $this->description() ); }

    public function APICall( $fields, $error ){ return APICallSub( '', $fields, $error ); }
    public function APICallSub( $sub, $fields, $error )
    {
        $call = Master::$cMaster->APICall('0.9.1/db/' . $this->db->ID . '/jobs/' . $this->ID + $sub, $fields );
